...
// @Security ApiKeyAuth
// @Router /user/{user_id}/devices/reset [post]
func ResetDevicesHandler(c *gin.Context) {
	...
	err = conn.ResetHwidSessions(ctx, userId)
	if err != nil {
		logger.Error(ctx, "failed to reset hwid sessions", zap.Error(err))
